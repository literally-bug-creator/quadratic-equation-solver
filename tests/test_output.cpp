...
TEST_CASE( "output(any, error_solution)", "[public][output]" ) {
    ...
    std::string error_message = random_string(DEFAULT_MAX_INT);
...
TEST_CASE( "output(any, linear) in cycle", "[public][output]" ) {
    for (int i = 0; i < RAND_TESTS_AMOUNT; i++){
        ...
        std::string expected_msg = SINGLE_ROOT_MSG + to_string(solution.x1) + "\n";
...
TEST_CASE( "output(any, two_roots) in cycle", "[public][output]" ) {
    for (int i = 0; i < RAND_TESTS_AMOUNT; i++){
        ...
        std::string first_root_msg = FIRST_ROOT_MSG + to_string(solution.x1) + "\n";
        std::string second_root_msg = SECOND_ROOT_MSG + to_string(solution.x2) + "\n";
...
TEST_CASE( "output(any, no_roots) in cycle", "[public][output]" ) {
    for (int i = 0; i < RAND_TESTS_AMOUNT; i++){
        ...
        std::string expected_msg = NO_ROOTS_MSG + "\n";
